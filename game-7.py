...
class Game:
    ...
    def start(self):
        ...
        while self.is_running:
            ...
            if self.turn == "player":
                direction = -direction  # Инвертируем направление, когда ход у игрока
            ...
            if self.monster.rect.left <= 0 or self.monster.rect.right >= self.screen_width:
                direction.x = 0  # Остановка движения по оси X
                direction.y = random.choice([-1, 1]) * abs(direction.y)  # Движение по оси Y
            if self.monster.rect.top <= 0 or self.monster.rect.bottom >= self.screen_height:
                direction.y = 0  # Остановка движения по оси Y
                direction.x = random.choice([-1, 1]) * abs(direction.x)  # Движение по оси X

            self.monster.rect.x += direction.x * self.monster.speed
            self.monster.rect.y += direction.y * self.monster.speed
            
            # Монстр атакует героя
            if self.turn == "monster" and self.monster.rect.colliderect(self.player.rect):
